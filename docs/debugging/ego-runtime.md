...
::: tip 阅读方法
**只记录本项目验证过的运行问题**，一个都没有编造。每条都写清楚：现象、怎么定位、真正原因、怎么处理、以及"为什么会发生"。
...
单机默认参数下这个错并不持续出现，规划器会在下一个周期重新尝试。所以本项目**没有改任何代码**。如果你把参数调大遇到它，两个方向：
...
**不要因为 launch 里写了就假设它生效了。** 这个问题在从 ROS 1 移植的项目里到处都是。
:::
...
- 一句话理解：本项目的运行期问题几乎全部来自"上游是从 ROS 1 移植过来的"，以及"容器默认什么都拿不到"。
