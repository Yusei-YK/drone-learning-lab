...
2026-09-16，用户报告 Docker 日志失控后无法进入系统，并已自行完成系统恢复。本轮停止飞行实验，只做取证、清理残留容器和防护验证。

**事故处置时的状态纠正：当时 PX4 v1.16.0 尚未完成真实升空、悬停、降落验收。** 前面对话把解锁、起飞服务返回成功、随后自动上锁写成“起飞闭环完成”，这一结论错误。已有数据中的高度没有达到目标。v1.15.4 的历史验收不能代替 v1.16.0 的验收。
...
两个实验容器已经停止，日志文件各为 **0 字节**。这部分恢复由用户在本轮之前完成，不能记成本轮释放了这些空间。容器退出码均为 `137`，但 Docker 的 `OOMKilled=false`；不能仅凭 137 就断言发生过 OOM。

【待验证】事故时日志峰值、磁盘被占满的准确时刻，以及无法登录的完整系统故障链。本轮没有原始大日志，不补写推测的容量和时间。
...
| 编译 | Ninja 显式限制为 2 个任务，容器另有限额；本轮没有编译 |
...
【待验证】恢复 v1.16.0 飞行实验前，先还原此前为解锁临时加入的预检/失联参数修改，核对固定模型版本，再验证带限额的单套仿真。防护自检通过不代表飞行通过。本轮事故修复不继续飞行。


## 6. 后续恢复实验结果（2026-09-16）

【运行验证】在上述防护启用后，恢复原厂 X500 配置、固定 Gazebo 子模块，并完成 v1.16.0 起降复验，自动脚本退出码 `0`，结束后两个容器已删除。详见 [PX4 第 13 节](/px4-sitl/environment#px4-116-acceptance)。这是新的运行证据，不使此前错误的“解锁等于起飞”结论成立。
