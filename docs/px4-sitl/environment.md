...
【源码确认】PX4 Makefile 把 `j` 变量传给 Ninja 的 `-j` 参数（v1.16.0：`Makefile:105`）。显式指定 `j=2` 限制编译任务数量；“再加 `-j` 会让并发翻倍”的旧解释不正确。容器 CPU 配额不等于编译任务数限制，两处都需要设置。后续 v1.16.0 复验只增量构建 16 个任务，恢复原厂启动配置并刷新正确版本号，没有重建镜像或全量 ROS 工作空间。
...
## 13. PX4 v1.16.0 原厂配置起降复验 {#px4-116-acceptance}
...
前提：当前电脑已有 `local/px4-humble-116:latest`、`~/Documents/Codex/px4-sitl-1.16/PX4-Autopilot` 的已编译产物及 `~/Applications/QGroundControl/AppRun`。这是本机验收入口，**不是新电脑的完整安装步骤**。不需要重建镜像，也不需要启动 EGO/VINS。

在 Ubuntu 桌面终端执行下面两行；第二行指定一个**尚不存在的目录**，重复运行时换一个后缀：

```bash
cd /home/yusei/Documents/Codex/drone-learning-lab
bash environments/px4-humble/accept-flight.sh "$HOME/Documents/Codex/px4-sitl-1.16/flight-check-01"
```

脚本顺序如下：

1. 通过受保护的 `run-sitl.sh` 启动唯一一套 SITL 与 MAVROS；复用已打开的 QGC，否则启动一个限时 QGC。
...
【运行验证】2026-09-16 最终完整运行退出码 `0`，[公开 JSON 证据](/evidence/px4-116-20260916.json)保留采样和事件，本机完整证据在：

```text
/home/yusei/Documents/Codex/px4-sitl-1.16/incident-20260916/flight-acceptance-4
```

| 检查 | 真实结果 |
| --- | --- |
| 初始地面真值 z | `-0.0130001844 m` |
| 起飞前 | `preflight: true`、`gcs: true`，未解锁且已着陆 |
| 解锁 / 起飞 / 降落应答 | 各 `success: true, result: 0`，仅作指令应答证据 |
| 连续悬停检查窗口 | `8.969 s`，真值 z `2.037～2.530 m`，均值 `2.414 m`，包含末段爬升收敛 |
| 悬停末 5 秒，2 Hz 采样 | 真值 z `2.449～2.530 m` |
| 降落前最后一帧 | EKF z `2.506 m`，垂直速度 `-0.0069 m/s` |
| 最终地面真值 z | `-0.0130003592 m` |
| 收尾 | `landed_state: 1`、`armed: false`，继续观测地面 5 秒 |
| 全程 | `42.06 s`，最终 `PASS`，程序正常退出，容器清理完成 |

其中 `landed_state: 2` 表示在空中，`1` 表示地面。悬停判据是连续至少 8 秒、离地高度 1.5～4 米、窗口高度极差小于 0.5 米、EKF 垂直速度绝对值小于 0.3 m/s；这是基础起降验收，不是高精度轨迹跟踪指标。

实际最后一条事件：

```json
{"t": 42.06, "phase": "ground_verify", "event": "PASS", "final_truth_z": -0.013000359192316152, "auto_disarmed": true}
```

要自己检查，在上述证据目录查看 `result.txt` 和 `flight.jsonl`：前者必须是 `exit_code=0`，后者必须有 `hover_pass`、最终 `PASS` 及前后位置变化。只看到服务返回成功不能算通过。

【运行验证】开发验收脚本时还实际遇到两个退出问题：首次飞行结束后 Gazebo Python 接收线程未清理，进程退出码 134；修复为退出前取消订阅并销毁节点。另一次全局文件大小限制被 QGC 缓存继承，QGC 被终止，预检拒绝放行；修复为仅限制证据写入进程。最终运行同时验证了正常退出和自动清理，保留失败证据，不把失败运行算作完整脚本通过。

【运行验证】最终证据目录共 `49,721` 字节。清理后 `docker ps -a` 为空；根分区可用约 `65G`，`MemAvailable` 约 `8.9Gi`，Swap **used 为 `4.0Ki`**，不是 4 GiB。没有重建镜像、删除已验收工作空间或做全局 Docker 清理。

### 13.5 应该记住什么，下一步是什么

记住：**服务成功 ≠ 物理升空；海拔 ≠ 离地高度；飞行动作完成 ≠ 验收程序正常退出。** 三者分别验证，结束还要检查资源与进程。

自测：目标是离地 2.5 米，能否直接在起飞服务里填 `altitude: 2.5`？

::: details 答案
不能把它当作通用写法。该参数是目标海拔；应正确换算目标海拔，或在已核对的 v1.16.0 实现中用非有限值选择「当前海拔 + 默认起飞高度」。还需验证真实物理高度确实上升。
:::

【待验证】下一节点是沿已验收的消息和坐标桥，把 px4ctrl 接到此基线，先做限幅、超时及状态机检查，再验收控制器悬停和 EGO 轨迹跟踪。本节完成后停在此处；D435/Mid360/Orin 与真实电机仍未部署或驱动。
