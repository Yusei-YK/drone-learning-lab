...
::: tip 这一页你会得到什么
本章建立后续实验共同使用的 ROS 2 Humble 容器环境。完成后，你将知道源码、构建产物和运行容器分别位于哪里。
...
| `ros-humble-pcl-ros` | PCL 与 ROS 消息的桥接 | 编译报找不到 `pcl_ros`（这是常见问题） |
...
中间会下载几 GB 的 apt 包。**中途失败绝大多数是网络问题，不是配置问题**——本项目遇到的问题一就是代理，详见 [构建期问题](/debugging/docker-build)。失败后重跑 `./build-image.sh`，Docker 会复用已完成的层，不用从头开始。
:::
...
手敲长命令迟早会漏参数，而漏掉的往往正是容易遗漏的参数。`environments/ego-humble/` 下的三个脚本将这些参数固化在脚本中，**直接在仓库里运行即可**，不需要抄到别处。
...
这就是「环境配好了」的真正含义：**不是我这台机器能跑，而是常见问题不会再被漏掉。**
:::
