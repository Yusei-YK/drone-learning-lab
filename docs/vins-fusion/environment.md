...
如果只想先完成运行验收，照着第 3、5、6、7 节走就行。第 4 节是这个镜像建了 6 次才成的记录，**急着跑通可以跳过**，遇到报错再回来查。
:::
...
这一节说明镜像依赖的作用，以及遇到构建问题时的定位方法。每个问题按“现象 → 原因 → 处理”组织。
...
开始时盯着 ROS 环境查了很久，因为报错里有 `ros/humble` 这个路径，看起来像"ROS 装坏了"。其实 ROS 一点问题都没有。
:::

### 4.2 问题二：Ceres 版本不对，而且**不能靠改代码绕过**
...
一种直觉做法是改代码适配 2.0（毕竟只是改个类名？）。查完发现**不能这么干**——`Manifold` 不是零星用了几处，而是贯穿全部四个包【源码确认】：
...
::: warning 最终选择不改代码的关键理由（这条比 Ceres 本身重要）
...
### 4.3 问题三：为了下 Ceres 加了代理，结果把 apt 搞坏了
...
### 4.4 问题四：`--network host` 把宿主机坏掉的 IPv6 也带进了容器
...
### 4.5 问题五（真正的根因）：这个网络封了 Ubuntu 官方源的 80 端口
...
### 4.6 问题六：`wget: not found`，退出码 127
...
### 4.7 问题七：长构建被"连坐"杀掉，以及顺手堵掉的一个 OOM 隐患
...
原因：将构建放在一个跟着当前会话的后台任务里。会话中断时，整个进程组被 SIGKILL 连坐带走了。**Docker 构建本身没有任何问题。**
...
这次日志里**没有** OOM 的痕迹，所以这次不是 OOM——但这属于"这次运气好"。将它改成了 `-j6`，做成 build arg 可覆盖：
